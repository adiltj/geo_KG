...
def main():
    # 分类名称
    # label_map = {0: '工程概况', 1: '探测内容', 2: '检测依据', 3: '主要仪器及设备', 4: '探测原理', 5: '探测方法',
    #              6: '隧道掌子面地质情况', 7: '探测结果', 8: '结论', 9: '建议'}
    ...
    # evaluation=''
    # for i,item in enumerate(classified):       
    #     if item == 7 and len(data[i])>15:
    #         evaluation=evaluation.join(data[i])
    #         evaluation=evaluation.replace(" ",'')

    # procaste=''
    # for i,item in enumerate(classified):       
    #     if item == 8 and len(data[i])>15:
    #         procaste=procaste.join(data[i])
    #         procaste=procaste.replace(" ",'')

    # advise=''
    # for i,item in enumerate(result):       
    #     if item == 9 and len(data[i])>15:
    #         advise=advise.join(data[i])
    #         advise=advise.replace(" ",'')

    # Print or return the index dictionary
    
    # words=''
    # words=words.join(face_info)
    # words=words.replace(" ",'')
    #结果打印
    print(f'-----label:{label_map[6]}-----\n')
    ...
    #     for k, v in r.items():
    #         if k in template:
    #             template[k]=template[k]+','+v
    # risk=riskEvaluation(template)
